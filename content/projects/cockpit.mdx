...
<ProjectTabs>
  ...
  <ProjectTab label="Low speed">
    In urban traffic at 40 mph attention is broader, stops are frequent, context changes fast. Range, signal timing, and parking guidance matter here. The driver has capacity to process more, but only if it's organized.
  </ProjectTab>
</ProjectTabs>
...
<ProjectAccordion>
  ...
  <ProjectAccordionItem title="Was this tested with real users?">
    Testing was internal: **a fixed-base simulator setup**, **3 participants**, **6 sessions** across **3 driving scenarios**.
  ...
  <ProjectAccordionItem title="Why not a heads-up display?">    
    The cluster remains the primary surface for comprehensive vehicle status even in HUD-equipped vehicles. HUD design also involves optical constraints that require a separate design process entirely:
  ...
  <ProjectAccordionItem title="What would come next?">
    Designing and testing the motion layer is the next phase of this project. Three areas were designed but animated only with basic crossfades in this iteration:

    - Transitions between driving modes;
    - Peripheral band fade;
    - Center panel switching between navigation and media.
