...
Both cards in that stack read from the same widget family, sized to sit together on one Home Screen row.
...
Fuel was the simplest card, and the first one tested across sizes.
...
Engine status is the simplest card in the set, a single word and an icon.
